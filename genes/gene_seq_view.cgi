...
##############################
#
# 4/14/03 - get GO information from RGD database, Lan Zhao
# 
##############################
...
sub display_html {
  ...
  my $html = RGD::HTML->new(title => "RGD Sequence Report: $data{gene_symbol}",
                            doc_title => "Sequence Report for $data{gene_symbol}",
                            version   => "1.0",
                            link_dir  => "genes",
                           );
...
#
 ...
 print "<tr><td colspan=2>&nbsp;</td></tr>"; 
  print qq[<tr class=\"sectionHead\"><td colspan=3><img src=\"/objectSearch/images/icon-l.gif\">&nbsp;<a name=\"References\" id=\"References\">Sequence Accessions</a></td></tr>];
  
   print qq[<tr valign="top"><td class="subsectionTitle">Nucleotide</td><td>];

 print qq[<table>];
  if($xdbs{$id}{1}) {
  	my @acc= split (/::/, $xdbs{$id}{1}{acc_id});
  	foreach my $a (0 .. $#acc) {
  		my $fastaURL = sprintf("https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Search&db=Nucleotide&term=%s[accn]&doptcmdl=FASTA",$acc[$a]);
		my $geoURL = sprintf("https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=geo&cmd=search&term=%s",$acc[$a]);
	  	my $xdbURL = "$xdbs{$id}{1}{xdb_url}"."$acc[$a]";
	  	print qq[<tr class=\"xdbRow\"><td>$xdbs{$id}{1}{xdb_name}:</td><td><A HREF=\"$xdbURL\">$acc[$a]</A></td><td><a href=\"$fastaURL\">Get FASTA</a></td><td><a href=\"$geoURL\">Search GEO Microarray Profiles</a></td></tr>];
  	}
  	}
print qq[</table></td></tr>];

print qq[<tr valign="top"><td class="subsectionTitle">Protein</td><td>];

 print qq[<table>];
  if($xdbs{$id}{7}) {my @acc= split (/::/, $xdbs{$id}{7}{acc_id});
  	foreach my $a (0 .. $#acc) {
  		my $fastaURL = sprintf("https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Search&db=Protein&term=%s[accn]&doptcmdl=FASTA",$acc[$a]);
	  	my $xdbURL = "$xdbs{$id}{7}{xdb_url}"."$acc[$a]";
	  	print qq[<tr class=\"xdbRow\"><td>$xdbs{$id}{7}{xdb_name}:</td><td><A HREF=\"$xdbURL\">$acc[$a]</A></td><td><a href=\"$fastaURL\">Get FASTA</a></td></tr>];
  	}
}
...
# sub get_gene_homolog_number {
# 
#   my $id = shift @_;
# 
#   my %results = ();
# 
#   my $sql = "select count(homolog_key) 
#               from rgd_hmlg_rgd_id 
#  where rgd_id = ?";
#   
#   my $sth = $db->{dbh}->prepare($sql) or die "Can't Prepare statement: $DBI::errstr";
#   
