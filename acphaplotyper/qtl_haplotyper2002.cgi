...
################
#
# qtl_haplotyper2002.cgi (c) Simon Twigger 1999-2002
#
...
###############
#
# Consolidate process_markers() and calculate_oldest_allele() subroutines?
#
#
#
#
...
###############
#
# 3/19/01 - Moved to RGD 2.0 Development site, adapted to work off RGD schema and
# appropriate flat files. Simon T.
#
#
#
###############
...
my $baseURL=$rgd_html->get_baseURL;   # http://rgd.mcw.edu
my $baseCGI=$rgd_html->get_baseCGI;   # http://rgd.mcw.edu/tools
...
my $SCRIPT_NAME = "PS_qtl_haplotyper";
my $GHOSTSCRIPT = "/opt/GS5.50/bin/gs";
...
if($form->param("load_action") eq "finish") {
  ...
  print "<P>If you wish to run further analyses you can <a href=\"/ACPHAPLOTYPER\">return to ACP Haplotyper</a> or to <a href=\"/ACPHAPLOTYPER/qtl_haplotyper.shtml\">return to QTL Haplotyper</a>\n";
...
my $user_email = $form->param('email') || "simont\@mcw.edu";
...
window.location.href = 'http://dorado.brc.mcw.edu/tools/acphaplotyper/qtl_haplotyper2002.cgi?load_action=finish';
//--></SCRIPT>
__JS__
...
# 2>/dev/null pipes any error messages to a null device
  ...
  my $pdf_file = $$ . "_acp_out.pdf";

  open (GS, "|$GHOSTSCRIPT -sDEVICE=pdfwrite -sOutputFile=$pdf_file -q - 2>/dev/null");
...
sub mail {
  ...
  my $top = MIME::Entity->build(
				...
				From => "simont\@mcw.edu",
				To => "$user_email",
				Subject => "QTL Haplotyper Result",
			       );

  my $message = <<"EOF";

Attached to this email is the output from RGD QTL Haplotyper
http://rgd.mcw.edu/ACPHAPLOTYPER/qtl_haplotyper.shtml
...
sub HTML_Error {
    ...
    print <<"EOF";
<HTML><HEAD><TITLE>PostScript QTL_HAPLOTYPER Message</TITLE></HEAD<
<BODY BGCOLOR="#FFFFFF">
<BR>
<H1>PostScript QTL_HAPLOTYPER Message</H1>
...
<HR>
Questions and Comments to <a href="mailto:simont\@mcw.edu">Simon Twigger</a>
</BODY></HTML>
EOF
